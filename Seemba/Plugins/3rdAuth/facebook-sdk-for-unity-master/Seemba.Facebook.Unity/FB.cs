...
namespace Seemba.Facebook.Unity
{
    using System;
    using System.Collections.Generic;
    using System.Globalization;
    using Seemba.Facebook.Unity.Gameroom;
    using Seemba.Facebook.Unity.Canvas;
    using Seemba.Facebook.Unity.Editor;
    using Seemba.Facebook.Unity.Mobile;
    using Seemba.Facebook.Unity.Mobile.Android;
    using Seemba.Facebook.Unity.Mobile.IOS;
    using Seemba.Facebook.Unity.Settings;
    using UnityEngine;
    ...
    public sealed class FB : ScriptableObject
    {
        ...
        {
            Init(
                Settings.FacebookSettings.AppId,
                Settings.FacebookSettings.ClientToken,
                Settings.FacebookSettings.Cookie,
                Settings.FacebookSettings.Logging,
                Settings.FacebookSettings.Status,
                Settings.FacebookSettings.Xfbml,
                Settings.FacebookSettings.FrictionlessRequests,
                authResponse,
            ...
            InitDelegate onInitComplete = null)
        {
            ...
            if (!isInitCalled)
            {
                ...
                if (Constants.IsEditor)
                {
                    ...
                    {
                        ((Editor.EditorFacebook)FB.facebook).Init(onInitComplete);
                    };

                    ComponentFactory.GetComponent<Editor.EditorFacebookLoader>();
                ...
                else
                {
                    switch (Constants.CurrentPlatform)
                    {
                        case FacebookUnityPlatform.WebGL:
                            FB.OnDLLLoadedDelegate = delegate
                            {
                                ((Unity.Canvas.CanvasFacebook)FB.facebook).Init(
                                    appId,
                                    cookie,
                                    logging,
                                    status,
                                    xfbml,
                                    Settings.FacebookSettings.ChannelUrl,
                                    authResponse,
                            ...
                            ComponentFactory.GetComponent<Unity.Canvas.CanvasFacebookLoader>();
                            break;
                        case FacebookUnityPlatform.IOS:
                            FB.OnDLLLoadedDelegate = delegate
                            {
                                ((Unity.Mobile.IOS.IOSFacebook)FB.facebook).Init(
                                    appId,
                                    frictionlessRequests,
                                    Settings.FacebookSettings.IosURLSuffix,
                                    onHideUnity,
                                    onInitComplete);
                            };
                            ComponentFactory.GetComponent<Unity.Mobile.IOS.IOSFacebookLoader>();
                            ComponentFactory.GetComponent<CodelessCrawler>();
                            ComponentFactory.GetComponent<CodelessUIInteractEvent>();
                            break;
                        case FacebookUnityPlatform.Android:
                            FB.OnDLLLoadedDelegate = delegate
                            {
                                ((Unity.Mobile.Android.AndroidFacebook)FB.facebook).Init(
                                    appId,
                                    onHideUnity,
                                    onInitComplete);
                            };
                            ComponentFactory.GetComponent<Unity.Mobile.Android.AndroidFacebookLoader>();
                            ComponentFactory.GetComponent<CodelessCrawler>();
                            ComponentFactory.GetComponent<CodelessUIInteractEvent>();
                            break;
                        case FacebookUnityPlatform.Gameroom:
                            FB.OnDLLLoadedDelegate = delegate
                            {
                                ((Unity.Gameroom.GameroomFacebook)FB.facebook).Init(appId, onHideUnity, onInitComplete);
                            };
                            ComponentFactory.GetComponent<Unity.Gameroom.GameroomFacebookLoader>();
                            break;
                        default:
        ...
        {
          #if UNITY_ANDROID
          var androidFacebook = FacebookImpl as Unity.Mobile.Android.AndroidFacebook;
        ...
        private static void LogVersion()
        {
            ...
            if (facebook != null)
            {
                FacebookLogger.Info(string.Format(
                    "using Seemba.Facebook Unity SDK v{0} with {1}",
                    FacebookSdkVersion.Build,
                    FB.FacebookImpl.SDKUserAgent));
            }
            else
            {
                FacebookLogger.Info(string.Format("using Seemba.Facebook Unity SDK v{0}", FacebookSdkVersion.Build));
            }
        }
        ...
        public sealed class Mobile
        {
            ...
            private static Unity.Mobile.IMobileFacebook MobileFacebookImpl
            {
                get
                {
                    Unity.Mobile.IMobileFacebook impl = FacebookImpl as Unity.Mobile.IMobileFacebook;
                    if (impl == null)
                    {
        ...
        {
            private static Unity.Gameroom.IGameroomFacebook GameroomFacebookImpl
            {
                get
                {
                    Unity.Gameroom.IGameroomFacebook impl = FacebookImpl as Unity.Gameroom.IGameroomFacebook;
                    if (impl == null)
                    {
