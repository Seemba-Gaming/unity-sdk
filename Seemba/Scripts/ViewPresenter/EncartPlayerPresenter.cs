...
public class EncartPlayerPresenter : MonoBehaviour 
{
	...
	public static void Init() 
	{
		...
		//	if(float.Parse(UserManager.Get.GetCurrentMoneyCredit())>0) {
  ...
  //          try{
		...
		//		PlayerAvatar.sprite = UserManager.Get.CurrentAvatarBytesString;
		//}catch(NullReferenceException ex){
		//}
  //          Texture2D txt1=new Texture2D(1,1);
		...
		//	if(!PullToRefresh.isActivated){
		//		if (!string.IsNullOrEmpty (UserManager.Get.CurrentUser.username)){
		//			PlayerUsername.text = UserManager.Get.CurrentUser.username;
		//		}
		//		else {
		...
		//	PlayerWater.text = int.Parse(UserManager.Get.GetCurrentBubblesCredit()).ToString();
		//}catch(NullReferenceException ex){}
		//catch(ArgumentNullException ex){}
		//try{
		...
		//	}
		//	else PlayerMoney.text = float.Parse(UserManager.Get.GetCurrentMoneyCredit()).ToString("N2")+" "+ CurrencyManager.CURRENT_CURRENCY;
		//}catch(NullReferenceException ex){
		//}
		//catch(ArgumentNullException ex){
		//}
